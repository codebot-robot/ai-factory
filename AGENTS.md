...
7. **Event Triggers:** Agents can be triggered by GitHub events. Note that `@codebot-robot` is the current robot to add to trigger things. For example, assigning an issue to the robot triggers it to solve the issue. Requesting a PR review from the robot triggers the `reviewer` agent to auto-review and approve the PR. Additionally, the execution of the `reviewer` agent is integrated with GitHub Checks and runs automatically on each Pull Request to validate specs and plans.
