...
10. **Tools:** Executable tools that agents can use are stored in the `tools/` directory. For example, `tools/run-subagent` is used to run a subagent within an agent sandbox, either immediately or as a long-running process that periodically wakes up based on an interval.
